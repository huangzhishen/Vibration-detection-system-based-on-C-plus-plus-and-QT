 ...
 */
void invpacktree::new_level( packdata<float> *elem )
{
  size_t half = elem->length();
  size_t n = half * 2;

  packcontainer *container = new packcontainer( n );
  container->lhsData( (float *)elem->getData() );
  stack.add( container );
} // new_level
...
void invpacktree::reduce()
{
  ...
  */
  ...
  float *vec = (float *)mem_pool.pool_alloc( n * sizeof( float ) );
  for (int i = 0; i < n; i++) {
    vec[i] = (*tos)[i];
  }
 ...
 */
void invpacktree::add_elem( packdata<float> *elem )
{
  assert( elem != 0 );
  ...
  if (stack.first() == 0) {
    ...
    if (tos->length() == n) {
      ...
      tos->rhsData( (float *)elem->getData() );
      reduce();
    }
 ...
 */
invpacktree::invpacktree( packdata_list<float> &list, 
			  liftbase<packcontainer, float> *w )
{
  ...
  packdata_list<float>::handle h;
  for (h = list.first(); h != 0; h = list.next( h )) {
    packdata<float> *elem = list.get_item( h );
    add_elem( elem );
  } // for
