...
/**
  Construct a wavelet packet tree from a vector of float
  ...
  \arg vec An array of float values on which the wavelet packet
 ...
 */
packtree::packtree( const float *vec, 
                    const size_t N, 
                    liftbase<packcontainer, float> *w )
{
  waveObj = w;


  float *vecCopy = (float *)mem_pool.pool_alloc( N * sizeof( float ) );
  stopLen = N / 8;
  for (int i = 0; i < N; i++) {
    vecCopy[i] = vec[i];
  }

  root = new packnode<float>( vecCopy, N, packnode<float>::OriginalData );
  root->mark( true );
 ...
 */
void packtree::cleanTree(packnode<float> *top, bool removeMark )
{
  if (top != 0) {
    if (removeMark) {
      if (top->mark()) {
 ...
 */
float packtree::bestBasisWalk( packnode<float> *top )
{
  float cost = 0.0;

  if (top != 0) {
    packnode<float> *lhs = top->lhsChild();
    packnode<float> *rhs = top->rhsChild();
    ...
    if (lhs == 0 && rhs == 0) { // we've reached a leaf
      ...
      float lhsCost = bestBasisWalk( lhs );
      float rhsCost = bestBasisWalk( rhs );

      float v1 = top->cost();
      float v2 = lhsCost + rhsCost;
 ...
 */
void packtree::checkBestBasis( packnode<float> *top )
{
  if (top != 0) {
    if (top->mark()) {
      foundBestBasisVal = true;
      if (top->getKind() == packdata<float>::OriginalData) {
 ...
 */
void packtree::buildBestBasisList( packnode<float> *top, 
				   packdata_list<float> &list )
{
  if (top != 0) {
    if (top->mark()) {
      list.add( top );
 ...
 */
packdata_list<float> packtree::getBestBasisList()
{
  packdata_list<float> list;
