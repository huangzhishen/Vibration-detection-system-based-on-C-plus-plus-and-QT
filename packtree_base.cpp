 ...
 */
void packtree_base::newLevel( packnode<float>* top, 
			      bool freqCalc, 
			      bool reverse )
{
  if (top != 0) {
	...
	waveObj->forwardStep( container, len );
      ...
      packnode<float> *lhs = new packnode<float>(container.lhsData(), 
						   len/2,
						   packnode<float>::LowPass );
      packnode<float> *rhs = new packnode<float>(container.rhsData(), 
						   len/2,
						   packnode<float>::HighPass );
...
void packtree_base::breadthFirstPrint(const printKind kind)
{
  ...
  Q.addQueue( root, 0 );
  while (! Q.queueEmpty() ) {
    packnode<float> *node = Q.queueStart()->node;
    ...
    packnode<float> *lhs = node->lhsChild();
    packnode<float> *rhs = node->rhsChild();
...
vector<vector<float>> packtree_base::getLevel3Data()
{
	queue<float> Q;
	...
	while (! Q.queueEmpty() ) {
		...
		if (nodeLen == stopLen)
		{
			const float *nodeData = node->getData();
			vector<float> tmp;
			for (int i = 0 ; i < nodeLen ;i++)
			{
				tmp.push_back(nodeData[i]);
			}
			res.push_back(tmp);
		}
